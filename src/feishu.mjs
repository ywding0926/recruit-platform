...
export async function getFeishuUserByCode(code) {
  ...
  const data = await res.json();
  if (data.code !== 0) throw new Error("飞书OAuth失败: " + data.msg);
  const d = data.data;
  return { id: d.open_id, name: d.name || d.en_name || "飞书用户", avatar: d.avatar_url || "", openId: d.open_id, unionId: d.union_id, provider: "feishu" };
}
...
export async function sendFeishuMessage(openId, content, title = "招聘平台通知") {
  ...
  try {
    const token = await getTenantAccessToken();
    const card = { config: { wide_screen_mode: true }, header: { title: { tag: "plain_text", content: title }, template: "blue" }, elements: [{ tag: "markdown", content }] };
    ...
    return data;
  } catch (e) { console.error("[Feishu] 发送消息异常:", e.message); return null; }
}
...
export async function createApprovalInstance(approvalCode, openId, formData) {
  ...
  try {
    ...
    const res = await fetch(`${FEISHU_HOST}/open-apis/approval/v4/instances`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({ approval_code: approvalCode, open_id: openId, form: JSON.stringify(formData) }),
    });
    const data = await res.json();
    if (data.code !== 0) console.error("[Feishu] 创建审批失败:", data.msg);
    return data;
  } catch (e) { console.error("[Feishu] 创建审批异常:", e.message); return null; }
}
